...
from davit import DaViT_even_tinyer as DaViT
from simplenet import simplenetv1_small_m1_05
import argparse

# hyperparameters
ENV_NAME = 'ALE/MsPacman-v5' # which game you want to play
...
class DqnAgent:
    ...
    def __init__(self, device, run_id_to_resume: int = None):
        self.device = device
        self.atari = AtariEnv()
        ...
        #    num_classes=self.atari.amount_actions,
        #    img_size=244).to(device)

        self.model = simplenetv1_small_m1_05(
            in_chans=AMOUNT_INPUT_FRAMES,
            num_classes=self.atari.amount_actions,
        ).to(device)
        ...
        #self.target_network = DaViT(
        #    in_chans=AMOUNT_INPUT_FRAMES,
        #    num_classes=self.atari.amount_actions,
        #    img_size=244
        #    ).to(device)

        self.target_network = simplenetv1_small_m1_05(
            in_chans=AMOUNT_INPUT_FRAMES,
            num_classes=self.atari.amount_actions,
        ).to(device)
        ...
        self.logger = TrainLogger(self, run_id_to_resume=run_id_to_resume)
        self.frames_trained = 0 # Track frames at the agent level

    def load_from_checkpoint(self, checkpoint):
        """Loads agent and logger state from a checkpoint dictionary."""
        ...
        self.epsilon.curr_step = epsilon_state['curr_step']
        self.epsilon.epsilon = epsilon_state['epsilon']
        print(f"Resuming with Epsilon: {self.epsilon.get():.4f} at frame step {self.epsilon.curr_step}")

        # Load logger state
        logger_state = checkpoint['logger_state']
        self.logger.episode_scores = logger_state['episode_scores']
        self.logger.running_avg_scores = logger_state['running_avg_scores']
        self.logger.frame_log = logger_state['frame_log']
        self.logger.episode_log = logger_state['episode_log']
        self.logger.current_best = logger_state['current_best']
        print(f"Resuming with {len(self.logger.episode_log)} episodes logged.")
        print(f"Current best evaluation score was: {self.logger.current_best:.2f}")

        # Return start episode number for the training loop
        start_episode = len(self.logger.episode_log)
    ...
    def compute_y(self, batch):
        ...
        q_value_sum += q_values_new.max(1)[0]

        # r_j + γ * max_a′ Q(φj+1, a′; θ)
        y = reward + self.gamma * q_value_sum * (1 - done)
    ...
    def train(self, start_episode=0):
        ...
        while self.frames_trained < TOTAL_FRAMES: # for episode = 1, M do
            ...
            while True: # for t = 1, T do
                ...
                if len(self.memory) > BATCH_SIZE:
                    ...
                    if self.frames_trained % UPDATE_TARGET_FREQ == 0:
                        self.target_network.load_state_dict(self.model.state_dict())

                # evaluate model and save every eval_every_frames frames
                if self.frames_trained > 0 and self.frames_trained % EVAL_EVERY_FRAMES == 0:
                    self.logger.eval_and_save()

                if done:
                    episode_num += 1
                    self.logger.log(episode_num, self.frames_trained, episode_reward)
                    break

class TrainLogger:
    # <<< MODIFIED TrainLogger __init__ >>>
    def __init__(self, agent: DqnAgent, run_id_to_resume: int = None):
        self.agent = agent
        ...
        if run_id_to_resume is not None:
    ...
    def log(self, episode_num, frame_num, episode_reward):
        ...
        print(
            f"Ep: {episode_num} | Frames: {self.agent.frames_trained} | Reward: {episode_reward:.1f} | "
    ...
    # <<< ADDED _save_checkpoint helper method >>>
    def _save_checkpoint(self, path):
        """Saves a comprehensive checkpoint for resuming training."""
        checkpoint = {
            'frames_trained': self.agent.frames_trained,
            'model_state_dict': self.agent.model.state_dict(),
            'target_network_state_dict': self.agent.target_network.state_dict(),
            'optimizer_state_dict': self.agent.optim.state_dict(),
            'epsilon_state': {
                'curr_step': self.agent.epsilon.curr_step,
                'epsilon': self.agent.epsilon.epsilon
            },
            'logger_state': {
                'episode_scores': self.episode_scores,
                'running_avg_scores': self.running_avg_scores,
    ...
    def save(self):
        ...
        # 3. Save the final model state (for easy inference)
        final_model_path = os.path.join(self.save_dir, 'final_model.pth')
        torch.save(self.network.state_dict(), final_model_path)
        print(f"Final model saved to {final_model_path}")

        # 4. Save the final checkpoint (for resuming)
        final_checkpoint_path = os.path.join(self.save_dir, 'final_checkpoint.pth')
        self._save_checkpoint(final_checkpoint_path)
        print(f"Final checkpoint for resuming saved to {final_checkpoint_path}")

        print("--- Save complete! ---")

    # <<< MODIFIED eval_and_save method >>>
    def eval_and_save(self):
        ...
        """
        print(f"\n--- Starting Evaluation (Frame {self.agent.frames_trained}) ---")
        ...
        # Save a checkpoint on every evaluation
        latest_checkpoint_path = os.path.join(self.save_dir, 'latest_checkpoint.pth')
        self._save_checkpoint(latest_checkpoint_path)
        ...
        if average_score > self.current_best:
            ...
            print(f"Score of {average_score:.2f} did not beat current best of {self.current_best:.2f}. Not saving best model.")
...
class AtariEnv:
    ...
    def step(self, action):
        ...
        # TAKE LOG REWARDS for pacman
        clipped_reward = np.log(full_reward) if full_reward > 0 else 0
        # Return a numpy array of stacked uint8 frames
...
def preprocess_frames(frame):
    ...
    downsampled_frame = cv2.resize(gray_frame, (244, 244))
    # Crop and keep as uint8
    return downsampled_frame


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DQN Training for Atari Games")
    parser.add_argument('--resume', type=str, default=None,
                        help='Path to a checkpoint file to resume training from (e.g., models/0/ALE.../latest_checkpoint.pth).')
    ...
    if args.resume:
        ...
        try:
            # Expected path structure: ".../models/<run_id>/<env_name>/<file>"
            path_parts = args.resume.replace('\\', '/').split('/')
            run_id_index = path_parts.index('models') + 1
            run_id_to_resume = int(path_parts[run_id_index])
            print(f"Resuming in run directory for Run ID: {run_id_to_resume}")
        except (ValueError, IndexError, TypeError):
            print("Warning: Could not automatically determine Run ID from path. A new run directory will be created if it doesn't exist.")

        agent = DqnAgent(device, run_id_to_resume=run_id_to_resume)
        checkpoint = torch.load(args.resume, map_location=device, weights_only=False)
        start_episode = agent.load_from_checkpoint(checkpoint)
    ...
    try:
        agent.train(start_episode=start_episode)
    finally:
        # The save method will now save the final model and checkpoint
        agent.logger.save()
