...
class DaViT(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, depths=(1, 1, 3, 1), patch_size=4,
                 ...
                 img_size=224, drop_rate=0., attn_drop_rate=0., **kwargs
                 ):
        super().__init__()
...
@register_model
def DaViT_tiny(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None, **kwargs):
    model_kwargs = dict(
...
@register_model
def DaViT_small(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None, **kwargs):
    model_kwargs = dict(
        patch_size=4, window_size=7, embed_dims=(96, 192, 384, 768), num_heads=(3, 6, 12, 24),
...
@register_model
def DaViT_base(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None, **kwargs):
    model_kwargs = dict(
...
@register_model
def DaViT_large_window12_384(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None, **kwargs):
    model_kwargs = dict(
...
@register_model
def DaViT_even_tinyer(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None, **kwargs):
    """
    A significantly smaller version of DaViT, designed for faster training on tasks like Atari.
    This model has reduced embedding dimensions, fewer attention heads, a shallower depth,
    and a smaller MLP ratio, resulting in a much lighter architecture.
    """
    model_kwargs = dict(
        patch_size=4,
        window_size=7,
        embed_dims=(64, 128, 256, 512),  # Reduced from (96, 192, 384, 768)
        num_heads=(2, 4, 8, 16),  # Reduced from (3, 6, 12, 24)
        depths=(1, 1, 2, 1),  # Reduced depth in the 3rd stage from 3 to 2
